...
enum kdl_identifier_emission_mode {
    KDL_PREFER_BARE_IDENTIFIERS, // Traditional: quote identifiers only if
...
struct kdl_emitter_options {
    ...
    kdl_float_printing_options
        float_mode;      // How to print floating point numbers
...
KDL_NODISCARD KDL_EXPORT kdl_emitter *
kdl_create_buffering_emitter(kdl_emitter_options const *opt);
// Create an emitter that writes by calling a user-supplied function
KDL_NODISCARD KDL_EXPORT kdl_emitter *
kdl_create_stream_emitter(kdl_write_func write_func, void *user_data,
...
KDL_EXPORT bool kdl_emit_node_with_type(kdl_emitter *emitter, kdl_str type,
                                        kdl_str name);
...
KDL_EXPORT bool kdl_emit_property(kdl_emitter *emitter, kdl_str name,
