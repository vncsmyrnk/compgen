...
enum kdl_character_set {
    KDL_CHARACTER_SET_V1 =
        1, // V1 character set: BOM is whitespace, vertical tab is not, etc.
    KDL_CHARACTER_SET_V2 =
        2, // V2 character set: control characters restricted, etc.
...
KDL_NODISCARD KDL_EXPORT kdl_tokenizer *
kdl_create_string_tokenizer(kdl_str doc);
// Create a tokenizer that reads data by calling a user-supplied function
KDL_NODISCARD KDL_EXPORT kdl_tokenizer *
kdl_create_stream_tokenizer(kdl_read_func read_func, void *user_data);
// Destroy a tokenizer
KDL_EXPORT void kdl_destroy_tokenizer(kdl_tokenizer *tokenizer);

// Change the character set used by the tokenizer
KDL_EXPORT void kdl_tokenizer_set_character_set(kdl_tokenizer *tokenizer,
                                                kdl_character_set cs);

// Get the next token and write it to a user-supplied structure (or return an
// error)
