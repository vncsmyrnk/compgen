...
// Parameter for kdl_escape - which characters should be escaped, and which
// should not?
enum kdl_escape_mode {
    ...
    // "Sensible" default: escape tabs, newlines, and other control characters,
    // but leave unicode intact
    KDL_ESCAPE_DEFAULT =
        KDL_ESCAPE_CONTROL | KDL_ESCAPE_NEWLINE | KDL_ESCAPE_TAB
};

enum kdl_version { KDL_VERSION_1, KDL_VERSION_2 };

// Function pointers used to interface with external IO
typedef size_t (*kdl_read_func)(void *user_data, char *buf, size_t bufsize);
typedef size_t (*kdl_write_func)(void *user_data, char const *data,
                                 size_t nbytes);
...
KDL_NODISCARD KDL_EXPORT kdl_owned_string kdl_escape_v(kdl_version version,
                                                       kdl_str const *s,
                                                       kdl_escape_mode mode);
// Resolve backslash escape sequences
KDL_NODISCARD KDL_EXPORT kdl_owned_string kdl_unescape_v(kdl_version version,
                                                         kdl_str const *s);
// Resolve backslash escape sequences and (in v2) dedent string
KDL_NODISCARD KDL_EXPORT kdl_owned_string
kdl_unescape_multi_line(kdl_version version, kdl_str const *s);

// Escape special characters in a string according to KDLv1 string rules
KDL_DEPRECATED("Use kdl_escape_v instead")
KDL_NODISCARD KDL_EXPORT kdl_owned_string kdl_escape(kdl_str const *s,
                                                     kdl_escape_mode mode);
