...
enum kdl_event {
    ...
    KDL_EVENT_START_NODE, // start of a node (a child node, if the previous node
                          // has not yet ended)
    ...
    //  - ORed with another event type: a node/argument/property that has been
    //  commented out
    //    using a slashdash
...
enum kdl_parse_option {
    ...
    KDL_READ_VERSION_2 = 0x40000,      // Use KDL version 2.0.0
    KDL_DETECT_VERSION = 0x70000,      // Allow both KDL v2 and KDL v1
    KDL_DEFAULTS = KDL_DETECT_VERSION, // Default: allow both versions
};
...
struct kdl_event_data {
    ...
    kdl_value value; // value including type annotation (for nodes: null with
                     // type annotation)
};

// Create a parser that reads from a string
KDL_NODISCARD KDL_EXPORT kdl_parser *
kdl_create_string_parser(kdl_str doc, kdl_parse_option opt);
// Create a parser that reads data by calling a user-supplied function
KDL_NODISCARD KDL_EXPORT kdl_parser *
kdl_create_stream_parser(kdl_read_func read_func, void *user_data,
                         kdl_parse_option opt);
// Destroy a parser
KDL_EXPORT void kdl_destroy_parser(kdl_parser *parser);

// Get the next parse event
// Returns a pointer to an event structure. The structure (including all strings
