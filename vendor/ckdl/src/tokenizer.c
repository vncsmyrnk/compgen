...
static inline kdl_utf8_status _tok_get_char(kdl_tokenizer *self,
                                            char const **cur, char const **next,
                                            uint32_t *codepoint);
...
kdl_tokenizer *kdl_create_stream_tokenizer(kdl_read_func read_func,
                                           void *user_data) {
    kdl_tokenizer *self = malloc(sizeof(kdl_tokenizer));
    if (self != NULL) {
...
void kdl_tokenizer_set_character_set(kdl_tokenizer *self,
                                     kdl_character_set cs) {
    self->charset = cs;
}

static size_t _refill_tokenizer(kdl_tokenizer *self) {
    if (self->read_func == NULL)
        return 0;
    ...
    size_t read_count =
        self->read_func(self->read_user_data, end, len_available);
...
bool _kdl_is_id(kdl_character_set charset, uint32_t c) {
    return _kdl_is_word_char(charset, c) &&
           !(charset == KDL_CHARACTER_SET_V2 && c == '#');
}

bool _kdl_is_id_start(kdl_character_set charset, uint32_t c) {
    return _kdl_is_word_start(charset, c) &&
           !(charset == KDL_CHARACTER_SET_V2 && c == '#');
}

bool _kdl_is_word_char(kdl_character_set charset, uint32_t c) {
    return c > 0x20 && c <= 0x10FFFF && c != '\\' && c != '/' && c != '(' &&
           c != ')' && c != '{' && c != '}' && c != ';' && c != '[' &&
           c != ']' && c != '"' && c != '=' &&
           !(charset == KDL_CHARACTER_SET_V1 &&
             (c == '<' || c == '>' || c == ',')) &&
    ...
    while (true) {
        ...
        if (_kdl_is_illegal_char(self->charset, c)) {
            ...
            while (_tok_get_char(self, &cur, &next, &c) == KDL_UTF8_OK &&
                   _kdl_is_whitespace(self->charset, c)) {
            ...
            if (c == '\r') {
                ...
                uint32_t c2;
                bool is_crlf =
                    _tok_get_char(self, &next, &nnext, &c2) == KDL_UTF8_OK &&
                    c2 == '\n';
            ...
            if (c == 'r' || c == '#') {
                ...
                if (rstring_status == KDL_TOKENIZER_OK)
                    return KDL_TOKENIZER_OK;
                // else: parse this as an identifier instead (which may also
                // fail)
            }
...
static kdl_tokenizer_status _pop_comment(kdl_tokenizer *self, kdl_token *dest) {
    ...
    char const *cur = self->document.data;
    char const *next = NULL;
    if (_tok_get_char(self, &cur, &next, &c1) != KDL_UTF8_OK)
        return KDL_TOKENIZER_ERROR;
    cur = next;
    if (_tok_get_char(self, &cur, &next, &c2) != KDL_UTF8_OK)
        return KDL_TOKENIZER_ERROR;
    if (c1 != '/')
        return KDL_TOKENIZER_ERROR;
...
static kdl_tokenizer_status _pop_string(kdl_tokenizer *self, kdl_token *dest) {
    ...
    if (_tok_get_char(self, &cur, &next, &c) != KDL_UTF8_OK)
        return KDL_TOKENIZER_ERROR;
    switch (c) {
    case 'r':
        is_raw = true;
    ...
    while (true) {
        ...
        if (_kdl_is_illegal_char(self->charset, c)) {
            ...
            hashes_found = 0;
                   ...
                   (hashes_found != 0 || quotes_found == initial_quote_count)) {
            ++hashes_found;
            quotes_found = 0;
        } else {
    ...
    // end_quote_ptr = the (first) end quote, next = the char after the quote
    // and hashes
    ...
    if (initial_quote_count == 3) {
        dest->type = is_raw ? KDL_TOKEN_RAW_MULTILINE_STRING
                            : KDL_TOKEN_MULTILINE_STRING;
    } else if (is_raw) {
