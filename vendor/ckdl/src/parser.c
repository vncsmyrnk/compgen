...
#define _str_equals_literal(k, l)                                              \
    ((k).len == (sizeof(l "") - 1) &&                                          \
     0 == memcmp(("" l), (k).data, (sizeof(l) - 1)))

#define KDL_PARSE_OPT_VERSION_BITS KDL_DETECT_VERSION

#define _v1_only(self)                                                         \
    ((self->opt & KDL_PARSE_OPT_VERSION_BITS) == KDL_READ_VERSION_1)
#define _v1_allowed(self)                                                      \
    ((self->opt & KDL_READ_VERSION_1) == KDL_READ_VERSION_1)
#define _v2_only(self)                                                         \
    ((self->opt & KDL_PARSE_OPT_VERSION_BITS) == KDL_READ_VERSION_2)
#define _v2_allowed(self)                                                      \
    ((self->opt & KDL_READ_VERSION_2) == KDL_READ_VERSION_2)
...
kdl_parser *kdl_create_string_parser(kdl_str doc, kdl_parse_option opt) {
    ...
    if (self != NULL) {
        ...
        kdl_tokenizer_set_character_set(self->tokenizer,
                                        _default_character_set(self->opt));
    }
    return self;
}

kdl_parser *kdl_create_stream_parser(kdl_read_func read_func, void *user_data,
                                     kdl_parse_option opt) {
    kdl_parser *self = malloc(sizeof(kdl_parser));
    if (self != NULL) {
        _init_kdl_parser(self, opt);
...
static bool _parse_value(kdl_parser *self, kdl_token const *token,
                         kdl_value *val, kdl_owned_string *s);
static bool _parse_number(kdl_str number, kdl_value *val, kdl_owned_string *s);
static bool _parse_decimal_number(kdl_str number, kdl_value *val,
                                  kdl_owned_string *s);
static bool _parse_decimal_integer(kdl_str number, kdl_value *val,
                                   kdl_owned_string *s);
static bool _parse_decimal_float(kdl_str number, kdl_value *val,
                                 kdl_owned_string *s);
static bool _parse_hex_number(kdl_str number, kdl_value *val,
                              kdl_owned_string *s);
static bool _parse_octal_number(kdl_str number, kdl_value *val,
                                kdl_owned_string *s);
static bool _parse_binary_number(kdl_str number, kdl_value *val,
                                 kdl_owned_string *s);
...
kdl_event_data *kdl_parser_next_event(kdl_parser *self) {
    ...
    while (true) {
        ...
        if (self->have_next_token) {
            ...
            case KDL_TOKENIZER_EOF:
                if ((self->state & 0xff) == PARSER_IN_NODE) {
                    ...
                    _set_parse_error(
                        self,
                        "Unexpected end of data (unclosed lists of children)");
                    return &self->event;
        ...
        if (token.type == KDL_TOKEN_NEWLINE &&
            self->state & PARSER_FLAG_NEWLINES_ARE_WHITESPACE) {
...
static kdl_event_data *_next_node(kdl_parser *self, kdl_token *token) {
    ...
    if (self->state & PARSER_FLAG_TYPE_ANNOTATION_START) {
        ...
        case KDL_TOKEN_END_TYPE:
            self->state = (self->state & ~PARSER_FLAG_TYPE_ANNOTATION_END) |
                          PARSER_FLAG_TYPE_ANNOTATION_ENDED;
            return NULL;
        default:
        ...
        case KDL_TOKEN_SEMICOLON:
            ...
            if (self->state & PARSER_MASK_NODE_CANNOT_END_HERE) {
                _set_parse_error(
                    self, "Unexpected end of node (incomplete node name?)");
                return &self->event;
            } else {
                // When there is no open type annotation, additional newlines
                // are allowed
                return NULL;
            }
        ...
        case KDL_TOKEN_RAW_MULTILINE_STRING:
            ...
            if (tmp_val.type == KDL_TYPE_STRING) {
                ...
                if (self->waiting_type_annotation.data != NULL) {
                    self->event.value.type_annotation =
                        self->waiting_type_annotation;
                    self->waiting_type_annotation = (kdl_str){NULL, 0};
                }
                ++self->depth;
        ...
        case KDL_TOKEN_LINE_CONTINUATION:
            // line continuations do nothing outside of nodes in v2, but are
            // illegal here in v1
            if (_v2_allowed(self)) {
                _set_version(self, KDL_VERSION_2);
                return NULL;
...
static kdl_event_data *_next_event_in_node(kdl_parser *self, kdl_token *token) {
    ...
    if (token->type == KDL_TOKEN_LINE_CONTINUATION) {
        ...
        if (token->type == KDL_TOKEN_EQUALS) {
            ...
            self->state = (self->state & ~PARSER_FLAG_MAYBE_IN_PROPERTY) |
                          PARSER_FLAG_IN_PROPERTY;
            ...
            if (self->state & PARSER_FLAG_CONTEXTUALLY_ILLEGAL_WHITESPACE) {
                self->state &= ~PARSER_FLAG_CONTEXTUALLY_ILLEGAL_WHITESPACE;
                if (_v1_only(self)) {
                    _set_parse_error(self,
                                     "Whitespace not allowed before = sign");
                    return &self->event;
                } else {
                    _set_version(self, KDL_VERSION_2);
                }
        ...
        case KDL_TOKEN_SEMICOLON:
            if (self->state & PARSER_MASK_NODE_CANNOT_END_HERE) {
                _set_parse_error(self, "Unexpected end of node (incomplete "
        ...
        if (self->state & PARSER_FLAG_END_OF_NODE &&
            self->slashdash_depth < 0) {
        ...
        if (self->state & (PARSER_FLAG_END_OF_NODE |
                           PARSER_FLAG_END_OF_NODE_OR_CHILD_BLOCK)) {
        ...
        case KDL_TOKEN_RAW_MULTILINE_STRING: {
            if (self->state & PARSER_FLAG_WHITESPACE_REQUIRED) {
                _set_parse_error(
                    self, "Whitespace required before argument or property");
                return &self->event;
            }
            // Either a property key, or a property value, or an argument.
            if (!_parse_value(self, token, &self->event.value,
            ...
            if (self->waiting_type_annotation.data == NULL &&
                (self->state & PARSER_FLAG_IN_PROPERTY) == 0 &&
                self->event.value.type == KDL_TYPE_STRING) {
...
static bool _parse_value(kdl_parser *self, kdl_token const *token,
    ...
    case KDL_TOKEN_STRING: {
        ...
        if (_v1_allowed(self))
            v1_str = kdl_unescape_v1(&token->value);
        if (_v2_allowed(self))
            v2_str = kdl_unescape_v2_single_line(&token->value);
    ...
    case KDL_TOKEN_WORD: {
        if (_str_equals_literal(token->value, "null")) {
        ...
        } else if (_v2_allowed(self) &&
                   _str_equals_literal(token->value, "#null")) {
            _set_version(self, KDL_VERSION_2);
            val->type = KDL_TYPE_NULL;
            return true;
        } else if (_v2_allowed(self) &&
                   _str_equals_literal(token->value, "#true")) {
            _set_version(self, KDL_VERSION_2);
            val->type = KDL_TYPE_BOOLEAN;
            val->boolean = true;
            return true;
        } else if (_v2_allowed(self) &&
                   _str_equals_literal(token->value, "#false")) {
            _set_version(self, KDL_VERSION_2);
            val->type = KDL_TYPE_BOOLEAN;
            val->boolean = false;
            return true;
        } else if (_v2_allowed(self) &&
                   _str_equals_literal(token->value, "#inf")) {
            _set_version(self, KDL_VERSION_2);
            ...
            return true;
                   ...
                   _str_equals_literal(token->value, "#nan")) {
            _set_version(self, KDL_VERSION_2);
            val->type = KDL_TYPE_NUMBER;
            val->number.type = KDL_NUMBER_TYPE_FLOATING_POINT;
        ...
        if (token->value.len >= 1) {
            ...
            if (first_char >= '0' && first_char <= '9') {
                // first character after sign is a digit, this value should be
                // interpreted as a number
                return _parse_number(token->value, val, s);
            } else if (_v2_only(self) && first_char == '.' &&
                       token->value.len - offset >= 2) {
        ...
        bool is_v1_identifier =
            _v1_allowed(self) && _identifier_is_valid_v1(token->value);
        bool is_v2_identifier =
            _v2_allowed(self) && _identifier_is_valid_v2(token->value);
...
static bool _parse_decimal_integer(kdl_str number, kdl_value *val,
    ...
    if (number.len - i > 0 && number.data[i] == '_')
        goto error;
...
static bool _parse_decimal_float(kdl_str number, kdl_value *val,
    ...
    for (; i < number.len; ++i) {
        ...
        if (c == '.' && state == before_decimal) {
            ...
            if (number.len - i <= 1 || number.data[i + 1] == '_')
                return false;
    ...
    if (digits_before_decimal + digits_after_decimal <= 15 &&
        explicit_exponent < 285 && explicit_exponent > -285) {
        // "strip zeros"
...
static bool _parse_hex_number(kdl_str number, kdl_value *val,
    ...
    if (number.len - i < 3 || number.data[i] != '0' ||
        number.data[i + 1] != 'x' || number.data[i + 2] == '_')
        goto error;
    i += 2;
...
static bool _parse_octal_number(kdl_str number, kdl_value *val,
    ...
    if (number.len - i < 3 || number.data[i] != '0' ||
        number.data[i + 1] != 'o' || number.data[i + 2] == '_')
        goto error;
    i += 2;
...
static bool _parse_binary_number(kdl_str number, kdl_value *val,
    ...
    if (number.len - i < 3 || number.data[i] != '0' ||
        number.data[i + 1] != 'b' || number.data[i + 2] == '_')
        goto error;
    i += 2;
...
static bool _identifier_is_valid_v1(kdl_str value) {
    uint32_t c = 0;
    if (_kdl_pop_codepoint(&value, &c) != KDL_UTF8_OK ||
        !_kdl_is_id_start(KDL_CHARACTER_SET_V1, c)) {
        return false;
    }
    ...
    while (true) {
        ...
        case KDL_UTF8_OK:
            if (!_kdl_is_id(KDL_CHARACTER_SET_V1, c))
                return false;
            break;
        case KDL_UTF8_EOF:
            return true;
...
static bool _identifier_is_valid_v2(kdl_str value) {
    if (_str_equals_literal(value, "inf") ||
        _str_equals_literal(value, "-inf") ||
        _str_equals_literal(value, "nan") ||
        _str_equals_literal(value, "null") ||
        _str_equals_literal(value, "true") ||
        _str_equals_literal(value, "false")) {
        return false;
    }

    uint32_t c = 0;
    if (_kdl_pop_codepoint(&value, &c) != KDL_UTF8_OK ||
        !_kdl_is_id_start(KDL_CHARACTER_SET_V2, c)) {
        return false;
    }
    ...
    while (true) {
        switch (_kdl_pop_codepoint(&value, &c)) {
        case KDL_UTF8_OK:
            if (!_kdl_is_id(KDL_CHARACTER_SET_V2, c))
                return false;
            break;
        case KDL_UTF8_EOF:
            return true;
