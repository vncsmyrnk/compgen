...
_kdl_write_buffer _kdl_new_write_buffer(size_t initial_size) {
    return (_kdl_write_buffer){
        .buf = malloc(initial_size), .buf_len = initial_size, .str_len = 0};
}
...
bool _kdl_buf_push_chars(_kdl_write_buffer *buf, char const *s, size_t count) {
    if (buf->buf_len - buf->str_len < count) {
        size_t increment =
            BUFFER_SIZE_INCREMENT >= count ? BUFFER_SIZE_INCREMENT : count;
...
bool _kdl_buf_push_codepoint(_kdl_write_buffer *buf, uint32_t c) {
    ...
    if (buf->buf_len - buf->str_len < 4) {
        size_t increment =
            BUFFER_SIZE_INCREMENT >= 4 ? BUFFER_SIZE_INCREMENT : 4;
        buf->buf_len += increment;
        buf->buf = reallocf(buf->buf, buf->buf_len);
        if (buf->buf == NULL) {
            return false;
...
kdl_owned_string kdl_escape_v(kdl_version version, kdl_str const *s,
                              kdl_escape_mode mode) {
    switch (version) {
    case KDL_VERSION_1:
        return kdl_escape_v1(s, mode);
    case KDL_VERSION_2:
...
kdl_owned_string kdl_unescape_multi_line(kdl_version version,
                                         kdl_str const *s) {
    switch (version) {
    case KDL_VERSION_1:
        return kdl_unescape_v1(s);
...
kdl_owned_string kdl_escape_v1(kdl_str const *s, kdl_escape_mode mode) {
    ...
    while (true) {
        ...
        if (c == 0x0A && (mode & KDL_ESCAPE_NEWLINE)) {
            ...
            if (!_kdl_buf_push_chars(&buf, "\\r", 2))
                goto esc_error;
        } else if (c == 0x09 && (mode & KDL_ESCAPE_TAB)) {
            if (!_kdl_buf_push_chars(&buf, "\\t", 2))
                goto esc_error;
        } else if (c == 0x5C) {
            if (!_kdl_buf_push_chars(&buf, "\\\\", 2))
                goto esc_error;
            ...
            if (!_kdl_buf_push_chars(&buf, "\\b", 2))
                goto esc_error;
        } else if (c == 0x0C && (mode & KDL_ESCAPE_NEWLINE)) {
            if (!_kdl_buf_push_chars(&buf, "\\f", 2))
                goto esc_error;
    ...
    while (p != end) {
        if (*p == '\\') {
            ...
            case 'u':
                ...
                if (!_kdl_buf_push_codepoint(&buf, c))
                    goto unesc_error;
                ++p;
                break;
            default:
        ...
        if (!_kdl_buf_push_chars(&buf, start, (p - start)))
            goto unesc_error;
    }
...
kdl_owned_string _kdl_dedent_multiline_string(kdl_str const *s) {
    ...
    kdl_str norm_lf =
        (kdl_str){.data = buf_norm_lf.buf, .len = buf_norm_lf.str_len};

    // What's after the final newline?
    char const *final_newline = NULL;
    for (char const *p = norm_lf.data + norm_lf.len - 1; p >= norm_lf.data;
         --p) {
        if (*p == '\n') {
            final_newline = p;
            break;
    ...
    kdl_str indent =
        (kdl_str){.data = final_newline + 1,
                  .len = (norm_lf.data + norm_lf.len) - (final_newline + 1)};
    ...
    while (in < end) {
        ...
        if (is_ws) {
            ...
            *(out++) = '\n';
            ...
            if (line_len >= indent.len &&
                memcmp(in, indent.data, indent.len) == 0) {
    ...
    while ((status = _kdl_pop_codepoint(&escaped, &c)) == KDL_UTF8_OK) {
        if (_kdl_is_illegal_char(KDL_CHARACTER_SET_V2, c)) {
            ...
            while ((status = _kdl_pop_codepoint(&tail, &c)) == KDL_UTF8_OK &&
                   (_kdl_is_whitespace(KDL_CHARACTER_SET_V2, c) ||
                    _kdl_is_newline(c))) {
                // skip this char
                escaped = tail;
            ...
            if (!removed_whitespace) {
                ...
                if ((status = _kdl_pop_codepoint(&escaped, &c)) ==
                    KDL_UTF8_OK) {
...
kdl_owned_string _kdl_resolve_escapes_v2(kdl_str const *s) {
    ...
    while ((status = _kdl_pop_codepoint(&escaped, &c)) == KDL_UTF8_OK) {
        if (_kdl_is_illegal_char(KDL_CHARACTER_SET_V2, c)) {
            ...
            if (_kdl_pop_codepoint(&escaped, &c) != KDL_UTF8_OK)
                goto unesc_error;
            ...
            case 'u': {
                // u should be followed by {
                if (_kdl_pop_codepoint(&escaped, &c) != KDL_UTF8_OK || c != '{')
                    goto unesc_error;

                uint32_t r = 0;
                while ((status = _kdl_pop_codepoint(&escaped, &c)) ==
                       KDL_UTF8_OK) {
                    // parse hex
                    if (c == '}')
                        break;
                ...
                if (status != KDL_UTF8_OK)
                    goto unesc_error;
                if ((0xD800 <= r && r <= 0xDFFF) // only Unicode Scalar values
