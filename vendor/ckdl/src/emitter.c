...
static size_t _buffer_write_func(void *user_data, char const *data,
                                 size_t nbytes) {
...
kdl_emitter *kdl_create_stream_emitter(kdl_write_func write_func,
                                       void *user_data,
                                       kdl_emitter_options const *opt) {
    kdl_emitter *self = malloc(sizeof(kdl_emitter));
    if (self == NULL)
        return NULL;
    self->opt = *opt;
...
static bool _emit_quoted_str(kdl_emitter *self, kdl_str s) {
    kdl_owned_string escaped =
        kdl_escape_v(self->opt.version, &s, self->opt.escape_mode);
    bool ok = self->write_func(self->write_user_data, "\"", 1) == 1 &&
              self->write_func(self->write_user_data, escaped.data,
                               escaped.len) == escaped.len &&
              self->write_func(self->write_user_data, "\"", 1) == 1;
    kdl_free_string(&escaped);
    return ok;
}

static kdl_owned_string
_float_to_string(double f, kdl_float_printing_options const *opts) {
    ...
    if (isnan(f)) {
        ...
        kdl_str result =
            f < 0.0 ? kdl_str_from_cstr("#-inf") : kdl_str_from_cstr("#inf");
        return kdl_clone_str(&result);
    }
    ...
    buf.str_len += snprintf(buf.buf + buf.str_len, buf.buf_len - buf.str_len,
                            "%d", integer_part);
    ...
    while (f + pos != f &&
           f_so_far < f) { // while this digit makes a difference
        ...
        if (next_digit == 0) {
            ...
            fprintf(stderr,
                    "- ckdl WARNING - _float_to_string calculated digit > 9\n");
...
static bool _emit_number(kdl_emitter *self, kdl_number const *n) {
    ...
    case KDL_NUMBER_TYPE_INTEGER:
        ...
        return (int)self->write_func(self->write_user_data, int_buf, int_len) ==
               int_len;
    case KDL_NUMBER_TYPE_FLOATING_POINT:
        float_str = _float_to_string(n->floating_point, &self->opt.float_mode);
        ok = self->write_func(self->write_user_data, float_str.data,
                              float_str.len) == float_str.len;
        kdl_free_string(&float_str);
        return ok;
    case KDL_NUMBER_TYPE_STRING_ENCODED:
        return self->write_func(self->write_user_data, n->string.data,
                                n->string.len) == n->string.len;
    }
    return false;
}
...
static bool _emit_bare_string(kdl_emitter *self, kdl_str s) {
    ...
    if (self->opt.identifier_mode == KDL_QUOTE_ALL_IDENTIFIERS) {
        ...
        kdl_str tail = s;
        bool first = true;
        kdl_character_set charset = self->opt.version == KDL_VERSION_1
                                        ? KDL_CHARACTER_SET_V1
                                        : KDL_CHARACTER_SET_V2;
        bool require_ascii =
            self->opt.identifier_mode == KDL_ASCII_IDENTIFIERS ||
            (self->opt.escape_mode & KDL_ESCAPE_ASCII_MODE) ==
                KDL_ESCAPE_ASCII_MODE;
        while (KDL_UTF8_OK == _kdl_pop_codepoint(&tail, &c)) {
            if ((first && !_kdl_is_id_start(charset, c)) ||
                !_kdl_is_id(charset, c) || (require_ascii && c >= 0x7f)) {
                bare = false;
                break;
            }
            first = false;
...
#define _write_string_literal_ok(self, s)                                      \
    (self->write_func(self->write_user_data, ("" s ""), sizeof(s) - 1) ==      \
     sizeof(s) - 1)
...
bool kdl_emit_property(kdl_emitter *self, kdl_str name,
                       kdl_value const *value) {
...
bool kdl_emit_end(kdl_emitter *self) {
    while (self->depth != 0) {
        if (!kdl_finish_emitting_children(self))
            return false;
    }
    if (!self->start_of_line) {
        if (!_write_string_literal_ok(self, "\n"))
